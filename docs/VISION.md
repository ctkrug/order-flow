...
quote you saw. That's correct, but it does not create the moment where the
idea clicks.

The tools that would let you see it for real, such as a professional trading terminal or raw
...
Depthwalk is for finance and computer-science students learning market
microstructure without a trading desk. They have read that thin books create
slippage but have never watched an order cause it. No account, data subscription,
or notebook is required; they can open the lab and run an experiment.
...
shows it happening: levels highlight as they are consumed, and a running dollar-slippage
...
- A visitor can land on the page with zero setup and, within seconds, drag a size
  ...
  counter. The core interaction from the top of this doc works end to end.
...
- The page is a single static, self-contained site: no backend, deployable to any
  ...
  (see `docs/DESIGN.md`), rather than a functional but bare prototype.
